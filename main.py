...
def get_random_message(file_name):
    with open("auto_message_wp/wp_messages.txt", 'r', encoding='utf-8') as file:
...
phone_number = "+905071320819" #Mesaj göndermek istediğin telefon numarasını gir.

def send_random_message():
    message = get_random_message("auto_message_wp/messages.txt")
    send_message(phone_number, message)

# Her saat başı rastgele bir mesaj gönderme işlemini planlama
schedule.every().hour.at(":17").do(send_random_message)
while True:
